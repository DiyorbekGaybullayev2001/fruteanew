        ...
        try {
          ...
          if (!response.ok) {
            ...
            if (response.status === 400) {
              throw new Error('Invalid product ID or request');
            }
          }
        ...
        <div className="border-b w-full pb-[25px] border-green-200 mb-6">
          <div className="max-w-[860px] mx-auto flex gap-[50px] text-[14px]">
            <div className="relative cursor-pointer" onClick={() => setActiveTab(0)}>
              <h4 className={`font-medium ${activeTab === 0 ? 'text-green-600' : 'text-black'}`}>
                Mahsulot tavsifi
              </h4>
              {activeTab === 0 && (
                <span className="absolute w-full h-[1px] bg-green-600 -bottom-[25px] left-0"></span>
              )}
            </div>
            <div className="relative cursor-pointer" onClick={() => setActiveTab(1)}>
              <h4 className={`font-medium ${activeTab === 1 ? 'text-green-600' : 'text-black'}`}>
                Fikr-mulohazalar
              </h4>
              {activeTab === 1 && (
                <span className="absolute w-full h-[1px] bg-green-600 -bottom-[25px] left-0"></span>
          ...
          {activeTab === 0 && (
            <div>
              <p className="text-[14px]">
                {product.extraInfoUz || 'No description available'}
              </p>
            </div>
          )}
          {activeTab === 1 && (
